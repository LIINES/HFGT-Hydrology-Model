# load_simulation1.jl
# Run Code for Simulation 1: System with 1 Lake
# Author: Megan S. Harris
# Date: May 2025

 HDF5, SparseArrays, XLSX
...
file_path = "0-Data/1-IntermediateData/HDF5s/myLFES-Full-Simulation 1-Default-Base-2.hdf5"
...
include("../2-SimulateDifferential/differential-solution.jl")
...
lake_concentrations = concentration_buffers[1,:]

hfgt_conc = reshape(lake_concentrations, 1, :)
diff_conc = reshape(conc_lake_sol, 1, :)

# Stack them vertically to make a 2 × 53 matrix
combined_conc = vcat(hfgt_conc,diff_conc)

# Define names for each line
names_combined = ["Lake 1: HFNMCF Discrete Time Solution","Lake 1: CSTR Differential Solution"]

# Call your plotting function
plot_combined = plot_buffer_timeseries(combined_conc, collect(time),
    names_combined,
    "Time (weeks)",
    "Concentration (mg/L)",
    "Simulation 1: Lake Concentration over Time",
    "0-Data/2-FinalData/LakeConcentrations/sim1_lakeconc_compare",  # Note: no semicolon, this is positional
